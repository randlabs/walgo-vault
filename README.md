...
AssetAmount: mint amount. The total minted amount must be less or equal to the ALGO Vault balance substract the fees (including the fees generated by this tx). It should be equal to arg1 Tx0
...
Receiver: any account. The remaining Vault balance must be greater than the amount of wALGOs minted less the pending fees and 2 tx cost (one for this and one for the CloseOut)

Fee: MinTxnFee

Amount: amount of ALGOs to withdraw, it should be equal to arg1 Tx0
...
* Do not allow registration if the Vault has balance. If the user calls ClearState with some Vault balance and minted wALGOs, then he could use the balance and avoid returning the wALGOs. So, we need to punish anyone calling ClearState without using CloseOut
