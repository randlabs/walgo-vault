...
const DEPOSIT_FEE_GLOBAL_KEY = 'DF'
...
class VaultManager {
	constructor (algodClient, appId = 0, adminAddr = undefined, assetId = 0) {
		...
		// setDepositFee
		this.setDepositFee = async function (adminAccount, newFee) {
			let appArgs = []
			appArgs.push(new Uint8Array(Buffer.from(SET_DEPOSIT_FEE_OP)))
			appArgs.push(new Uint8Array(tools.getInt64Bytes(newFee)))

			return await this.callApp (adminAccount, appArgs)
		}

		this.depositFee = async function () {
			let ret = await vaultManager.readGlobalStateByKey(DEPOSIT_FEE_GLOBAL_KEY)
			if(!ret) {
				return 0
			}
			return ret
		...
		this.maxWithdrawAmount = async function (accountAddr) {
			let amount = await this.maxMintAmount(accountAddr) - this.minTransactionFee() - this.minVaultBalance()
			if(amount < 0) {
				return 0
			}
			return amount
		...
		this.maxMintAmount = async function (accountAddr) {
			...
			let amount = vaultBalance - minted - pendingFees
			return amount
		}
