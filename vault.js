...
const WITHDRAWAL_FEE_GLOBAL_KEY = 'WF'
...
const SET_WITHDRAWAL_FEE_OP = 'sWF'
...
class VaultManager {
	constructor (algodClient, appId = 0, adminAddr = undefined, assetId = 0) {
		...
		// setWithdrawalFee
		this.setWithdrawalFee = async function (adminAccount, newFee) {
			let appArgs = []
			appArgs.push(new Uint8Array(Buffer.from(SET_WITHDRAWAL_FEE_OP)))
			appArgs.push(new Uint8Array(tools.getInt64Bytes(newFee)))

			return await this.callApp (adminAccount, appArgs)
		}

		this.withdrawalFee = async function () {
			let ret = await vaultManager.readGlobalStateByKey(WITHDRAWAL_FEE_GLOBAL_KEY)
			if(!ret) {
				return 0
			}
			return ret
		...
		// pendingVaultRewardsFees: get unclaimed rewards fees generated by the Vault (vaultWithdrawalFees - withdrewVaultRewardsFees)
		// this.pendingVaultRewardsFees = async function (accountAddr) {
		// 	let fees = await this.vaultWithdrawalFees(accountAddr)
		// 	let withdrew = await this.withdrewVaultRewardsFees(accountAddr)
		// 	return (fees - withdrew)
		// }

		// vaultWithdrawalFees: get the total rewards fees generated by the Vault
		this.vaultWithdrawalFees = async function (accountAddr) {
			...
			let withdrawalFee = await this.withdrawalFee()

			let rewards = vaultBalance + withdrawals - deposits
			if(rewards > 0) {
				return Math.floor((vaultBalance + withdrawals - deposits) * withdrawalFee / 10000)
			}
			return 0
		}
